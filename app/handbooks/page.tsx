...
import { handbooksNav } from "@/lib/handbooks";
import { openHandbookSearch } from "@/components/Handbook/HandbookSearch";
import Image from "@/components/Common/Image";

export const metadata: Metadata = {
  title: "Handbooks",
  description:
    "Practical, plain-language handbooks from Reactify Solutions on the tools and methods we use. Claude Code is the first one. Free, no signup.",
  alternates: { canonical: "/handbooks" },
  openGraph: {
    title: "Handbooks | Reactify Solutions",
    description:
      "Practical, plain-language handbooks from Reactify Solutions on the tools and methods we use. Claude Code is the first one.",
    url: "/handbooks",
  ...
  twitter: {
    ...
    title: "Handbooks | Reactify Solutions",
    description: "Practical handbooks on the tools and methods we use. Free, no signup.",
  },
};

function countPages(topic: (typeof handbooksNav)[number]): number {
    ...
    <div className="relative overflow-hidden">
      ...
      <div className="container py-16 lg:py-20">
        ...
        <h1 className="font-display mb-5 max-w-[760px] text-4xl font-semibold leading-[1.08] text-white sm:text-5xl md:text-[52px]">
          Handbooks, in <span className="text-gradient-accent">plain language</span>.
        </h1>

        <p className="mb-14 max-w-[620px] text-lg leading-relaxed text-textSecondary">
          We write these for ourselves first, so each one is short, honest about
          what we are still figuring out, and built around a working example rather
          than a wall of theory. Every handbook covers one tool or method
          end to end. Pick one to begin.
        </p>

        <div className="flex flex-wrap gap-5">
          {handbooksNav.map((topic) => {
            const pageCount = countPages(topic);
            return (
              <Link
                key={topic.topic}
                href={`/handbooks/${topic.topic}`}
