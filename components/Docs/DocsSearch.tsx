...
type MiniSearchInstance = {
  ...
  ) => Array<DocSearchEntry & { id: string; score: number }>;
};

// Any header on any /docs page can open this without prop drilling across
...
// Mounted once, in app/docs/layout.tsx, so ⌘K and every search trigger on
// every /docs page share one instance and one lazily-loaded index.
export default function DocsSearch() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("docs-search:open", openHandler);
    return () => {
      window.removeEventListener("keydown", handler);
      window.removeEventListener("docs-search:open", openHandler);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const docs: DocSearchEntry[] = await res.json();
      if (cancelled) return;
      ...
      mini.addAll(docs.map((d) => ({ ...d, id: d.id })));
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-start justify-center px-4 pt-[92px]">
      ...
      <div className="relative w-full max-w-[620px] overflow-hidden rounded-2xl border border-darkBorderStrong bg-darkSurface shadow-[0_30px_80px_-20px_rgba(0,0,0,0.85)]">
        ...
        <div className="flex items-center gap-4 border-t border-darkBorder bg-docsCodeBg px-4 py-[9px]">
